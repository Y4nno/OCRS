...
func (r *subscriptionResolver) RegistrationAdded(ctx context.Context) (<-chan *model.Registration, error) {
	ch := make(chan *model.Registration, 1)
	r.mu.Lock()
	if r.subscriptionMap == nil {
		r.subscriptionMap = make(map[chan *model.Registration]bool)
	}
	r.subscriptionMap[ch] = true
	r.mu.Unlock()

	go func() {
		<-ctx.Done()
		r.mu.Lock()
		delete(r.subscriptionMap, ch)
		r.mu.Unlock()
		close(ch)
	}()
...
func (r *Resolver) BroadcastRegistration(reg *model.Registration) {
	r.mu.Lock()
	for subChan := range r.subscriptionMap {
		subChan <- reg
	}
	r.mu.Unlock()
}
...
func (r *mutationResolver) CreateRegistration(
  ...
  VALUES ($1, $2, $3, $4, $5, $6)`,
	...
	r.Resolver.BroadcastRegistration(newRegistration)
...
func (r *mutationResolver) CreateRegistrations(
	...
	for _, reg := range createdRegs {
		r.Resolver.BroadcastRegistration(reg)
	}
...
func (r *mutationResolver) UpdateRegistration(
	...
	updatedRegistration := &model.Registration{
		ID:        id,
		StudentID: studentID,
		CourseID:  courseID,
		Status:    status,
	...
	r.Resolver.BroadcastRegistration(updatedRegistration)
...
func (r *mutationResolver) CompleteCourse(ctx context.Context, studentID string, courseID string) (*model.Registration, error) {
	updatedAt := time.Now().Format(time.RFC3339)

	// Only allow marking as completed if currently enrolled
	result, err := r.Resolver.DB.Exec(`
        UPDATE registrations
        SET status = 'completed', updated_at = $1
        WHERE student_id = $2 AND course_id = $3 AND status = 'enrolled'`,
		updatedAt, studentID, courseID)
	if err != nil {
		return nil, fmt.Errorf("failed to mark course as completed: %v", err)
	}

	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		return nil, fmt.Errorf("no active enrollment found to complete for student %s and course %s", studentID, courseID)
	}
